...
class ConstNode(object):
    """
    Bias
    每一个Layer都有一个Bias项，且该Bias项不接收上游输出，且初始化值为1
    """
    def __init__(self, layer_index, node_index):
        self.layer_index = layer_index
        self.node_index = node_index
...
class Layer(object):
    ...
    def calc_output(self):
        for node in self.nodes[:-1]: # 剔除 Bias 神经元, Bias 是固定值
            node.calc_output()
...
class NetWork(object):
    def __init__(self, layers):
        ...
        for i in range(layer_count):
            self.layers.append(Layer(i, layers[i])) # 生成Layer，并生成当前层Node
        for layer in range(layer_count - 1): # 逐层添加 Connection
            connections = [Connection(upstream_node, downstream_node)
                           for upstream_node in self.layers[layer].nodes
                           for downstream_node in self.layers[layer + 1].nodes[:-1]] # Bias 神经元不接收输入
